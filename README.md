...
**The tribunals already exist; what's missing is the software.** DIFC and ADGM are real common-law courts inside special economic zones in the UAE; SICC is a division of the Singapore High Court. They publish their judgments, cite specific rules, and produce orders that enforce abroad: judgments through bilateral treaties, reciprocal-enforcement statutes and common-law recognition, and awards made under the same forum's arbitration rules through the New York Convention's 172 contracting states.[^nyc] What they don't yet have is a way for software to *replay their reasoning* — to take a fact pattern, run it through the rule the court applied, and surface the same answer.
...
Most "Web3 dispute resolution" proposals try to invent a tribunal from scratch. The thesis here is the inverse: **the tribunals already exist; what is missing is the computational layer.** DIFC and ADGM are common-law courts inside special economic zones in the United Arab Emirates; SICC is a division of the Singapore High Court staffed partly by international judges. All three operate with versioned rules, dated evidence, separation of powers, and rulings that carry an explicit enforcement bridge beyond the tribunal (reciprocal-enforcement statutes, UAE federal recognition, and, for arbitral awards, the New York Convention). This repository codes 188 judgments from the three tribunals against six per-ruling primitives a digital tribunal must satisfy, compiles **twelve rules from the corpus into executable Catala predicates and seven full case traces, six of which reproduce the courts' principal output exactly while the seventh bounds a discretion residue by design**, and exposes the deterministic predicate cores as a reusable rule library plus a public dashboard, a Postgres-backed read-only API, a schema-driven rule playground, and first-party Python + TypeScript clients.
...
| Cross-border enforcement (judgments) | UAE federal recognition; DIFC Practice Direction 2/2015 converts an unpaid judgment to an award only on both parties' prior written agreement | UAE Cabinet Resolution + Federal Law | Singapore *Reciprocal Enforcement of Commonwealth Judgments Act*; Hague Choice of Court 2005 where the clause is exclusive |
| Cross-border enforcement (arbitral awards) | New York Convention | New York Convention | New York Convention (IAA s 31) |
