...
We code 121 publicly-issued judgments from three operating special-jurisdiction commercial courts: the Dubai International Financial Centre (DIFC) Courts, the Abu Dhabi Global Market (ADGM) Courts, and the Singapore International Commercial Court (SICC). Each judgment is scored against six per-ruling primitives a digital tribunal must satisfy and two architectural system properties of the tribunal as a whole. 39 judgments form a hand-coded gold set; the other 82 are AI-triaged or AI-graded against the same v0.2 rubric, with per-entry provenance recorded. All three tribunals score at near-ceiling: SICC averages 1.95 / 2.00, ADGM 1.91, DIFC 1.72. The saturation pattern survives the order-of-magnitude expansion of the ADGM sample (1.93 at n=7 → 1.91 at n=76) and replicates across a third tribunal in a different legal family (English-law-via-statute vs Singapore common law). All three score 2/2 on both system properties. We then compile four rules from the corpus into Catala source[^catala] and a Python predicate evaluator, demonstrating coverage of (i) static formulae, (ii) deferred conditionals, (iii) bounded discretion, and (iv) arithmetic composition over substantive findings. The protocol reproduces the courts' principal numerical answers exactly in three of four traces; in the fourth, the protocol does not produce a single number by design but bounds the discretion residue. Two of the four traces surface a clerical or methodological gap in the court's order. The constructive contribution of this paper is the claim that **the tribunals already exist; what is missing is the computational layer**.
...
The contribution of this paper is to make that argument empirically rather than rhetorically. We code 121 actual rulings from three actual tribunals against a six-primitive framework, and we show that the framework saturates: most rulings, in most primitives, score at ceiling. We then write four of the rules in compilable form and run them against the case facts. Three of four traces reproduce the court's central numerical answer exactly; the fourth bounds a discretion residue to roughly 7% of the claim. Two of the four surface clerical or methodological gaps that the predicate makes mechanically visible.
...
## 5. Constructive results: four traces

The empirical layer measures whether the rules a tribunal applies have the right shape to be compiled. The constructive layer demonstrates the compilation. Each of the four traces lifts a rule from the gold set into Catala source plus a Python predicate evaluator and runs it against an event log of the case facts.
...
### 5.5 Across the four traces

| Trace | Shape | Tribunal action | Protocol output |
|---|---|---|---|
| #1 *Dhawan v El Jaouhari* | Pure formula | AED 7,127.75 (operative); AED 7,121.75 (schedule) | AED 7,121.75 + flags 6 AED clerical error |
| #2 *Oberlin v Ovidiu* | Deferred conditional | 14-day window, 9% p.a. retroactive if missed | "What is owed today?" deterministic across any as-of date |
| #3 *Taylor v Yao Affi* | Bounded discretion | AED 128,914.80 → AED 120,000 (–AED 8,914.80) | 1 disposed, 1 held to zero, 1 surfaced; AED 8,914.80 = structured residue |
| #4 *Projeco v Ideacrate* | Composition over findings | Net AED 10,500.96 + interest AED 877.48 | Net AED 10,500.96 exact; interest reveals +1-day daycount convention |

The four traces cover the spectrum of rule shapes a digital tribunal must handle: static formulae, deferred conditionals, bounded discretion (the honest case for what rules cannot fully decide), and arithmetic composition over substantive findings. The protocol reproduces the court's central numerical answer exactly in three of four; in the fourth (Trace #3) the protocol does not produce a single number by design but bounds the discretion residue to a computable percentage. Two of the four traces (Trace #1 and Trace #4) surface a clerical or methodological gap that the predicate makes mechanically visible.
...
**Doctrinal coverage of traces.** Four traces is enough for a coverage claim across the rule shapes (formula / conditional / bounded discretion / composition over findings) but does not cover the doctrinal spectrum. A future iteration would compile a jurisdictional-challenge decision (where the rule is itself a *meta-rule* about which rule applies) and a duty-of-care decision (where both the substantive determinations and the rule turn on doctrinal lines like Caparo proximity).
